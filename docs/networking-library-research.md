...
**调研日期：** 2026-08-10
**修订：** 2026-08-12 —— R1、R2 的目标平台收敛为 Linux 单平台，本文的平台门槛随之下调。选型结论不受影响：原推荐在单平台条件下同样成立。
**决策范围：** R2 局域网双人 P2P BO3；Linux；Bevy `0.19.x`；手动输入 `IP:端口`；60Hz 确定性模拟、回滚、状态校验与断线判负。

## 1. 结论

推荐采用 **`bevy_ggrs = "0.22"` + `ggrs = "0.13"` + GGRS 内置 UDP socket 与项目握手层**，并保持 `game_core` 为不依赖 Bevy 的确定性状态机。
...
本项目的规则状态已经被设计为独立的 `game_core::MatchState`。R2 应让 GGRS 回滚该状态，客户端从回滚后的最新快照重建视觉层；对局版本、规则版本、角色和种子的握手，以及断线判负，均保留在 `net` 中实现。该边界避免把 UI、音频、socket 和渲染实体误纳入可回滚状态。
...
| 目标平台与 CI | 依赖必须声明支持 Bevy 0.19，且能覆盖 Linux 构建与无窗口同步测试。 |
| 断线判负 | 需要连接状态/超时事件；规则层接收该事件并终止比赛。 |

评分含义：`5` 为直接满足；`3` 为少量适配即可满足；`1` 为同步模型或版本不匹配。总分只用于排序，最终决策以原型验收为准。

| 方案 | Bevy 0.19 | P2P 回滚 | 手动 LAN | 与纯 `game_core` | 实施量（分高为少） | 适配度 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
**项目适配。** 两个玩家、离散输入位集合、显式随机种子、纯整数状态与回放日志，均是回滚同步的理想输入。`MatchState: Clone` 可以成为快照边界；更推荐先把状态包装为一个已注册的 rollback resource，或在 GGRS 适配层实现快照/恢复，保持 `game_core` 无 Bevy 依赖。UDP 监听、加入、版本握手和断线超时属于 `net`，每端都运行相同逻辑。
...
**项目适配。** 将 `game_core` 作为确定性模拟，传输每 tick 输入，并把主机作为轻量服务器，可以完成 R2。其输入缓存和冗余输入包有助于应对 UDP 丢包。
...
**项目适配。** 该方案和现有 `game_core` 边界高度兼容，网络层只需编码 `InputFrame`、握手和心跳；host 可同时运行 server 与 client。它让 R2 保持最小依赖集，并方便精确控制协议。
...
**项目适配。** 为 R2 的确定性棋盘状态逐字段复制会绕开“双方相同输入导出相同状态”的核心设计；主机也会成为权威服务器。可以仅用其可靠事件层，但价值很低，且 `game_core` 需要更深地映射成 Bevy 组件/资源。
...
Bevy client
  ├─ 采集并量化本地输入 (60Hz)
  ├─ bevy_ggrs 固定帧调度
  │    └─ game_core::MatchState.step([P1Input, P2Input])
  ├─ 从最新回滚状态渲染 / 播放确认帧表现事件
  └─ net
...
| 数据 | 归属 | 回滚 | 传输 |
| --- | --- | --- | --- |
| `TickInput`（动作位与帧号） | `game_core` / GGRS 适配 | 是 | GGRS UDP |
| `MatchState`、RNG、计时器、垃圾/Fever 状态 | `game_core` | 是 | 本地快照；定期 checksum |
...
1. 将“确切兼容版本待定”改为：**原型目标为 `bevy_ggrs = "=0.22.0"` 与 `ggrs = "=0.13.0"`**；任何升级均以兼容矩阵和 CI 重新验证。
...
| Bevy 集成 | bevy_ggrs 0.22 | 目标平台构建失败，或插件调度无法与独立 `game_core` 保持边界。 |
| 传输 | GGRS 内置 LAN UDP | 增加 Web、NAT 穿透或互联网房间时评估 Matchbox / Lightyear。 |
| 架构保留 | `game_core` 纯确定性，`net` 基础设施，`client` 表现层 | 产品转向服务器权威、观战或持久世界时评估 Lightyear / Replicon。 |
