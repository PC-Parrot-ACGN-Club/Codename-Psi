...
| TC-010 | 绑定捕获的写入、取消与覆盖语义 | P1 | Component | — | Input；Configuration | 可进入捕获态并注入一次物理输入 | 参数化执行三种捕获过程 | 捕获到未被占用的物理位；捕获期间施加返回输入；捕获到同一玩家同设备已占用的物理位并选择覆盖 | 第一组写入新绑定；第二组取消捕获并保留原绑定；第三组产生具名冲突结果交给设置 UI，选择覆盖后新动作绑定成功且原先占用该物理位的动作变为未绑定；捕获态期间不产生 `UIAction` 也不产生规则动作 | [Confirmed] [本机用户设置：绑定捕获](../../development/design/user-settings.md#绑定捕获)、[输入绑定冲突](../../development/design/user-settings.md#输入绑定冲突) |
