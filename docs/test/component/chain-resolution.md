...
| TC-015 | 两个零时长边界都不为连锁步增加 tick | P1 | Component | — | Rules | 已冻结的结算时长配置与一条二连锁盘面 | 读取剖面时长；逐 tick 推进并记录每个 tick 的阶段标签 | `clear_preview_ticks=12`；重力表 1 格 10、2 格 15、3 格 19；TC-007 的盘面（本轮最大下落 2 格） | `12 + 10 = 22` 与 `12 + 19 = 31` 与 [DEC-004](../../development/decision/settlement-timing-values.md) 记录的连锁步区间一致；本盘面第 1 步为 `12 + 15 = 27` tick，第 12 tick 停在 `ClearCommit`、第 27 tick 停在 `ScanNext`，第 28 tick 已在为第 2 步的 `ClearPreview` 计入第 1 tick | [Confirmed] [连锁结算：阶段时长](../../development/design/chain-resolution.md#阶段时长)、[数据模型](../../development/design/chain-resolution.md#数据模型) |
