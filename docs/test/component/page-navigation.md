...
- 设置页的按键层级按玩家、设备、动作逐级下降，返回逐级上升（TC-010、TC-014）。
- 两个下角的按键提示分属两名玩家，给出各自当前的确认与返回键（TC-011）。
- 设置项的值与页面各行文字都随当前语言给出，数值除外（TC-012）。
- 设置页设备级的手柄项随手柄连接状态启用或禁用，项数不变，重建后仍带原因（TC-013）。
...
| 状态转移 | 设置页四级层级逐级下降再逐级上升 | TC-010、TC-014 |
| 场景法 | 改绑旋转前后的按键提示文本 | TC-011 |
| 对比测试 | 同一页面模型与同一份设置在两种语言下的文本 | TC-012 |
| 等价类划分 | 启用项与禁用项 | TC-003、TC-006、TC-013 |
| 场景法 | 层级升降前后重新施加设备可用性 | TC-013 |
...
| TC-010 | 设置页按键层级按玩家与设备逐级下降，返回逐级上升 | P1 | Component | — | Client | 已构造 Settings 焦点环，焦点在按键设置入口 | 依次确认「按键设置」「P2」「键盘」，再连续施加三次 `Back`，最后再施加一次 `Back` | 四级层级；两名玩家；两种设备类别；四个可配置动作 | 顶层依次给出通用设置项、按键设置、返回且不含任何绑定行；三次确认分别得到玩家级、设备级与绑定级，每级末项为返回；绑定级的项为该玩家该设备的四个可配置动作；三次 `Back` 逐级上升并把焦点落回进入该级时的那一项；回到顶层后再一次 `Back` 才产生目标为来源状态的 `SettingsClosed` | [Confirmed] [页面导航与焦点：设置页层级](../../development/design/page-navigation.md#设置页层级) |
...
| TC-013 | 设备级的手柄项随手柄连接状态启用或禁用 | P1 | Component | — | Client；Input | 已下降到某名玩家的设备级 | 依次置手柄为未连接、已连接，重复置一次相同状态；再置未连接并上升一级后重新下降 | 无手柄连接；一只手柄连接 | 未连接时手柄项禁用并给出随包语言目录中存在的不可用原因，键盘项不受影响，项数不变；禁用时确认该项不下降到绑定级；连接后恢复启用且不再带不可用原因；重复置相同状态时报告无变化；层级重建后手柄项仍为禁用且仍带原因 | [Confirmed] [页面导航与焦点：设备可用性](../../development/design/page-navigation.md#设备可用性) |
| TC-014 | 下级的返回项与返回输入同样只上升一级 | P2 | Component | — | Client | 已从顶层下降到玩家级，焦点落在该级的返回项 | 施加一次 `Confirm` | 玩家级的返回项 | 不产生任何迁移请求；层级回到顶层且焦点落回按键设置入口 | [Confirmed] [页面导航与焦点：设置页层级](../../development/design/page-navigation.md#设置页层级) |
