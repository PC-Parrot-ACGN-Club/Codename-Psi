...
- 设置页的手柄重绑定项随手柄连接状态启用或禁用，行数不变（TC-013）。
...
| TC-013 | 手柄重绑定项随手柄连接状态启用或禁用 | P1 | Component | — | Client；Input | 已构造 Settings 焦点环 | 依次置手柄为未连接、已连接，并重复置一次相同状态 | 无手柄连接；一只手柄连接 | 未连接时全部手柄重绑定项禁用并给出随包语言目录中存在的不可用原因，键盘重绑定项不受影响，项数不变；连接后这些项恢复启用且不再带不可用原因；重复置相同状态时报告无变化 | [Confirmed] [页面导航与焦点：设备可用性](../../development/design/page-navigation.md#设备可用性) |
