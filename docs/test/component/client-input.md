...
- 一名玩家同时只由一个输入源驱动，手柄接入与断开都即刻改变它（TC-010）。
...
| 状态迁移 | 同一名玩家在键盘与手柄之间来回切换输入源 | TC-010 |
...
| TC-003 | 多个固定物理来源产生同一动作时合并为一个逻辑动作 | P1 | Component | — | Input；Client | 同一输入源内两个物理位均可产生 Left | 两来源同时处于 pressed 并采样 | 十字键 Left + 左摇杆 X 轴负向越过阈值 | raw actions 仅含一个 Left 位，无冲突或重复 | [Confirmed] [本地输入采样：合并多个物理输入源](../../development/design/local-input-sampling.md#合并多个物理输入源)；[UI 交互动作：物理绑定关系](../../development/design/ui-action-input.md#物理绑定关系) |
...
| TC-010 | 一名玩家同时只由一个输入源驱动 | P0 | Component | — | Input；Client | 装有真实设备读取的客户端 app，两名玩家均使用默认绑定且均无手柄 | 对局中先用键盘产生 P1 的方向动作，接入一只手柄后分别注入仍按住的键与新按下的键，再用手柄产生方向动作，随后断开手柄并注入键盘输入；另在菜单上下文对同一序列取 `UIAction` | 一只手柄；P1 与 P2 的默认键盘键；手柄十字键 | 无手柄时键盘驱动 P1；手柄接入后 P1 的键盘输入不再产生任何逻辑动作，仍按住的键与新按下的键都不产生，手柄输入照常产生；手柄断开后键盘即刻恢复驱动；全程 P2 不受影响，仍由键盘驱动；菜单上下文得到同样的结论，`UIAction` 只来自该玩家当前的输入源 | [Confirmed] [本地输入采样：设备与玩家绑定](../../development/design/local-input-sampling.md#设备与玩家绑定)、[输入源](../../development/design/local-input-sampling.md#输入源)；[UI 交互动作：绑定来源表](../../development/design/ui-action-input.md#绑定来源表) |

## 风险查漏

六种 GameAction、输入上下文、玩家隔离、多来源、输入源切换、edge 和阈值均有直接用例；生产 schedule 与设备断连风险由集成测试稿覆盖。
