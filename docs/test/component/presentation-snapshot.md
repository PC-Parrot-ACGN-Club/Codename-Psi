...
- 圆框角色的姿态按固定优先级从规则事实中选出（TC-011）。
- 一条连锁的最终连锁数落入四个 cut-in 分档之一，1 连不落入任何分档（TC-012）。
...
| 判定表 | Chain 结算（垃圾窗口空/非空）× `NuisanceDropped` × 溢出风险 × 对方溢出风险的组合 | TC-011 |
| 边界值 | 文字行存活期的最后一 tick 与其后一 tick | TC-008 |
| 边界值 | 队列为 0、恰好一个单位、单位减一与远超面板槽位 | TC-009 |
| 边界值 | 四个分档的上下边界与 0/1 连、顶档无上限 | TC-012 |
...
| TC-011 | 圆框角色的姿态按固定优先级从规则事实中选出 | P2 | Component | — | Client | 可构造任意快照与姿态事件窗口 | 逐项加上更高优先级的事实，每次取一次姿态 | 无事实；对方溢出风险；再加己方溢出风险；`NuisanceDropped` 事件窗口（不伴随溢出风险）；窗口到期；`ChainSettled` 且垃圾窗口非空/为空；`ChainSettled` 与己方溢出风险同时成立 | 依次得到待机、优势、受伤（自身溢出风险覆盖优势）、受伤（事件窗口内，窗口到期后落回待机）、`offset`/`spell`（按垃圾窗口是否非空区分）；`ChainSettled` 同时成立时仍取 `offset`/`spell`，不降级为受伤 | [Confirmed] [表现运行时：圆框角色演出](../../development/design/presentation-runtime.md#圆框角色演出)；[表现与 UI 设计 §3.1、§4.5](../../presentation.md) |
| TC-012 | 一条连锁的最终连锁数落入固定的四个 cut-in 分档之一 | P2 | Component | — | Client | 无（纯函数） | 参数化对若干连锁数各求一次分档 | 0、1、2、3、4、5、6、7、8、`u8::MAX` | 0 与 1 不落入任何分档；2/3 同档，4/5 同档，6/7 同档；8 与 `u8::MAX` 同属最高档，顶档无上限 | [Confirmed] [表现运行时：连锁与 Fever cut-in](../../development/design/presentation-runtime.md#连锁与-fever-cut-in)；[表现与 UI 设计 §3.1](../../presentation.md#cut-in) |

## 风险查漏

快照完整性、事件编号与去重、可重建性、优势推导、文字反馈取舍、队列分级、题面预设标记、圆框姿态、cut-in 分档与动画强度不变性均有直接用例；跳帧采样、降级与整局 checksum 一致性由集成测试稿覆盖。cut-in 的实际触发时机（末连 `ClearCommit` 前瞻）与立绘渲染尚未实现，分档只是渲染就绪后要取用的分类，不在本文范围。
