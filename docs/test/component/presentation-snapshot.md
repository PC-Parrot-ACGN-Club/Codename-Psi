...
| TC-011 | 圆框角色的姿态按固定优先级从规则事实中选出 | P2 | Component | — | Client | 可构造任意快照与文字行 | 逐项加上更高优先级的事实，每次取一次姿态 | 空队列；队列 12；再加溢出风险；再加送出 9 与抵消 4 的文字行；再加 Fever；再加 `Decided(1)`、`Draw` 与 `Completed(0)` | 依次得到待机、防御、紧张、进攻与抵消、Fever、失败与胜利；和局不指定胜负方，姿态落回下一项；胜负同时决定两侧 | [Confirmed] [表现运行时：圆框角色演出](../../development/design/presentation-runtime.md#圆框角色演出)；[表现与 UI 设计 §3.1、§4.5](../../presentation.md) |
