...
测试用例设计流程、审核文档模板与交付前检查见[测试用例设计约定](design/README.md)。
...
1. 用户当前任务与已确认决定。
2. 当前有效的 [PRD](../PRD.md)、[玩法设计](../gameplay.md) 与 [技术设计](../TDD.md)。
3. 已确认的审核结论。
4. 当前代码与配置。
5. 既有测试与 Git 历史。
...
[测试用例设计约定](design/README.md)是测试设计流程、审核文档模板、字段标准和交付前检查的唯一入口。
...
测试设计在实现前进入用户审核。审核确认 Test Basis、证据状态、分类、数据和预期结果后，用例进入实现与回归集合。设计变更同步更新关联用例的依据、预期结果与证据状态。
