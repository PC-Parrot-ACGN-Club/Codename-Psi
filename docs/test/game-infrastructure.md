...
| TC-065 | 键盘 `Escape` 按上下文产生 `Pause` 或 `Back` | P1 | Component Integration | — | Input；Client | 最小 Bevy App 已注册项目根插件 | 分别在 `Match` 与可返回页面状态下按下 `Escape` | `AppState::Match`；`AppState::MainMenu` | `Match` 下提出 `PauseRequested` 并进入 `Paused`；`MainMenu` 下不提出 `PauseRequested`，按 `Back` 语义处理 | [Confirmed] [应用状态机：协作](../development/design/application-state-machine.md#协作)；[UI 交互动作：全局 `Escape`](../development/design/ui-action-input.md#全局-escape) |
