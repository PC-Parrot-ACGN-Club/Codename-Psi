...
- 设置页双入口的返回上下文与同一状态边上的 cause 区分（TC-011～TC-012）。

## 设计方法与覆盖模型

| 方法 | 输入模型 / 风险 | 关联用例 |
| --- | --- | --- |
| 状态迁移 | 初始化、同状态、合法迁移、仲裁、完整主路径与返回上下文 | TC-001～TC-005、TC-008、TC-011～TC-012 |
...
| TC-002 | 同状态请求为 no-op 且不触发生命周期 | P1 | Component Integration | — | Client | 最小 Bevy App 为 OnExit/OnEnter 安装计数器 | 对八种状态分别请求自身并运行状态提交周期 | Boot→Boot … Result→Result，含 Settings→Settings | 当前状态保持；进入/退出计数不增；无非法边诊断 | [Confirmed] [应用状态机：请求处理](../../development/design/application-state-machine.md#请求处理) |
...
| TC-010 | 规则数据为 Failed 时不提出进入 Match 的迁移请求 | P0 | Component Integration | Content Validation | Configuration；Client | 最小 app 已到达可提出开局请求的流程点，规则数据 resolution 可注入 | 分别注入 `Loaded` 与 `Failed` 的规则数据并运行开局请求 system | `Failed(UnsupportedSchema)`；等价有效规则数据 | `Failed` 时不产生 `CharacterConfirmed`、`AppState` 不变且失败原因可观察；`Loaded` 时产生一次请求 | [Confirmed] [应用状态机：协作](../../development/design/application-state-machine.md#协作)；[版本化运行数据加载：失败分级](../../development/design/runtime-data-loading.md#失败分级) |
| TC-011 | 设置页从两个入口进入后各自返回来源状态 | P1 | Component Integration | — | Client | 最小 Bevy App 可提出 `SettingsOpened` 与 `SettingsClosed` | 分别从两个来源进入设置并返回 | 从 `MainMenu` 进入后返回；从 `Paused` 进入后返回 | 两次进入均提交到 `Settings`；返回分别提交到 `MainMenu` 与 `Paused`；返回目标取自进入前写入的 `SettingsOrigin`，不受另一次进入影响 | [Confirmed] [应用状态机：设置返回上下文](../../development/design/application-state-machine.md#设置返回上下文)；[有效状态转移](../../development/design/application-state-machine.md#有效状态转移) |
| TC-012 | `CommittedTransition` 记录本次迁移并区分同一状态边上的不同 cause | P0 | Component Integration | — | Client | 当前 `Paused`，消费者可在 `OnEnter(Match)` 读取记录 | 分别以两种 cause 提交 `Paused → Match` | `ResumeRequested`；`RestartRequested` | 两次提交后 `CommittedTransition` 的 `from`/`to` 均为 `Paused`/`Match`，`cause` 分别为两者；`OnEnter(Match)` 的消费者据此得到不同处置分支；非法请求被拒绝时不写入记录 | [Confirmed] [应用状态机：数据模型](../../development/design/application-state-machine.md#数据模型) |

## 风险查漏

单一顶层状态、生命周期次数、重复请求、优先级、启动屏障、降级、开局阻断、返回上下文与同边 cause 区分均有直接用例。
