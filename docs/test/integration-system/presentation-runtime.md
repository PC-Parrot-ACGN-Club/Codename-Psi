...
- 两块棋盘贴住中央通道排布，连锁数画在自己棋盘的区域内（TC-019～TC-020）。
...
- 棋盘空格底色随 Fever 开关切换，已占据的格子不受影响（TC-021）。
- Fever 题面重现按格子所在行自底向上分批唤起并落入终点，`Reduced` 强度只省略落入插值（TC-022）。
...
| 等价类划分 | 空格底色按 Fever 开/关；球格与垃圾格不受 Fever 影响 | TC-021 |
| 边界值分析 | 行唤起前 / 唤起瞬间 / 落入中 / 落地后四个时点；两档动画强度 | TC-022 |
...
| TC-012 | `ClearPreview` 与 `Gravity` 先停顿再按加速度投影，`Reduced` 改为吸附 | P1 | Component | — | Client | 可按阶段内的 tick 查询被消球的姿态与下落球的位置 | 逐 tick 取整段阶段上的姿态与位置，两档动画强度各取一次 | 阶段时长取整数 tick；同一批下落含落差不同的两颗球；`AnimationIntensity` 取 `Full` 与 `Reduced` | `Full` 下：停顿期间被消球保持原尺寸与不透明度、下落球停在起点；此后被消球完成闪光再单调缩小淡出，到阶段末已淡出；下落球按等加速度前进，落差最大的一颗在阶段末落地，落差小的更早落地并保持不动，位置单调不回退，越界 tick 仍停在终点。`Reduced` 下：被消球全程保持一次稳定高亮，下落球停在起点直到阶段结束再吸附到终点。两档均不改变 `duration_ticks` | [Confirmed] [表现运行时：画面重建](../../development/design/presentation-runtime.md#画面重建)、[动画强度](../../development/design/presentation-runtime.md#动画强度)；[表现与 UI 设计 §6.1](../../presentation.md) |
...
| TC-015 | 圆框下的名字来自 roster 与当前语言，而不是掉落组标识 | P1 | Component Integration | — | Client；Configuration | 客户端 app 装有 Bevy UI 栈，roster 与本地化目录均可用，对局已建立 | 进入对局并等待运行数据落地，读取屏上全部文本 | 双方角色为 `alpha`、`beta`；两者的 `display_name_key` 在当前语言目录中均有条目 | 屏上出现两个角色 `display_name_key` 在当前语言下的文本；不出现角色标识本身；键在目录中缺失时不以键名充当名字 | [Confirmed] [角色表现数据：边界](../../development/design/character-presentation.md#边界)；[本地化运行时](../../development/design/localization-runtime.md) |
...
| TC-019 | 两块棋盘贴住中央通道排布，尺寸与竖向预算符合版面 | P1 | Component Integration | — | Client | 客户端 app 装有 Bevy UI 栈，对局已建立且 UI 布局已计算 | 读取两侧全部棋盘格的实际尺寸与位置 | 随包剖面的 6 × 12 盘面 | 格为 60 px 见方；单块棋盘横跨 370 px；两块棋盘之间只隔中央通道；两块关于画布中线对称且都远离画面边缘；棋盘上方留出垃圾行、下方留出分数，整体在 1080 内 | [Confirmed] [表现与 UI 设计 §2.1](../../presentation.md) |
| TC-020 | 连锁数画在自己棋盘的区域内 | P1 | Component Integration | — | Client | 客户端 app 装有 Bevy UI 栈，对局已建立，一方盘面上有一个待触发的消除组 | 硬降触发消除，待连锁数出现后读取其位置与所属棋盘的范围 | 底行四颗同色球；`HardDrop` | 连锁数文本的横向范围落在自己棋盘的横向范围内，纵向位置落在棋盘的纵向范围内 | [Confirmed] [表现与 UI 设计 §3.1、§4.1](../../presentation.md) |
| TC-021 | 空格底色随 Fever 开关切换，占据格不受影响 | P2 | Component | — | Client | 可按占据者与是否处于 Fever 查询单格填充色 | 分别取空格与各类占据者在 Fever 开/关下的填充色 | `Cell::Empty`；`Cell::Color(0..5)`；`Cell::Nuisance`；`in_fever` 取开/关 | Fever 关闭时空格为常规棋盘底色；开启时空格改用与常规底色不同的 Fever 底色；球格与垃圾格的填充色不随 Fever 状态变化 | [Confirmed] [表现与 UI 设计 §4.1](../../presentation.md) |
| TC-022 | Fever 题面重现自底向上分行落入，`Reduced` 改为整行吸附 | P2 | Component | — | Client | 可按行、tick 与动画强度查询单格的题面重现取值 | 参数化取题面各行在起始 tick、行唤起前、落入过程中与落地后的绘制状态，两档动画强度各取一次 | 题面跨两行以上；行间隔与落入时长取固定 tick 值；`AnimationIntensity` 取 `Full` 与 `Reduced` | `Full` 下：最靠下的一行从起始 tick 即开始落入，更靠上的行各自延后被唤起；行唤起前该格按空绘制；唤起后的格子从上方短距离落入，随 tick 单调靠近终点，落入时长结束后偏移归零。`Reduced` 下：行唤起前仍按空绘制，唤起瞬间直接吸附到终点，不经过中间偏移 | [Confirmed] [表现运行时：题面重现](../../development/design/presentation-runtime.md#题面重现) |
