...
- 采样器输出可以直接交给 core::input 归一化，不包含设备相关数据。
...
- [Confirmed] 当前审核结论：本地输入处理作为独立采样 Component；持续动作按 fixed tick 边界的当前按下状态采样；硬降与旋转每次物理按下产生一次且不会因持续按住重复；逻辑动作冲突统一交给 core::input。
