...
| `core::config` | 从内存文本/字节解析 core typed model、schema/version 校验 | `serde`、RON/JSON |
...
3. source 交给 `core::config` 或对应 client parser。
...
`core::config` 返回领域解析错误；`client::data` 增加：
...
- `core::config`：只处理调用方传入的内存内容，不访问文件系统或 Bevy。
