...
- 处理：**先消耗活动通道的队列，溢出部分再消耗另一通道的队列**；两个通道都清空后仍有余量时形成送往对手的攻击。可抵消的数量以[进入安全点时的队列](match-and-round.md#安全点)为准。
...
- 处理：本次落子触发了至少一个连锁时不落下，玩家继续获得下一活动组；未触发连锁且队列非零时按单次上限取出一批，每颗从所在列最高的空格进入活动盘面。整行先填；不足一行时按[玩法设计 §4.2](../../gameplay.md) 的分支推进列序——凑满整行后余 1 颗则下一次首颗从下一列开始，余 2 颗及以上则下一次首颗从上一颗所在列开始。整列无空格时该颗不进入盘面。
- 输出：入场坐标与更新后的队列、列序位置。
- 错误语义：落下量不超过单次上限，余量留在队列。

这批垃圾随即走[连锁结算](chain-resolution.md#重力稳定)的重力流程：按本批最大下落格数查同一张时长表，期间盘面保持入场时的样子，时长走完才原子提交停稳后的盘面。落下期间玩家不持有活动组，下一活动组在垃圾停稳的那个 tick 才生成（见[安全点](match-and-round.md#安全点)）。
...
| 入场坐标与队列总数 | 本主题 | 表现层 | 精确整数；图标分级由表现层投影 |
| 垃圾落下的重力移动与进度 | 本主题 | 表现层 | 与连锁重力同一个[表现协议](chain-resolution.md#表现协议)，表现层不区分两者 |
