...
- 输入：该玩家的 Fever 状态、待接收垃圾、溢出风险、[文字反馈](#文字反馈)当前的行、优势方、小局与比赛结果，以及该角色的[表现数据](character-presentation.md)。
- 处理：按固定优先级取一个姿态——胜负 → Fever → 刚离场的攻击或抵消 → 溢出风险 → 有待接收垃圾 → 优势 → 待机；和局不指定胜负方，因此落到下一项。姿态给出该状态的循环运动与立绘；一次性运动由事件驱动，与姿态各自独立。
- 输出：两侧圆框的配色、立绘、当前循环运动，以及本 tick 触发的一次性运动。
- 错误语义：角色表现目录不可用时用[替补](character-presentation.md#查询)绘制，只失去角色间的区分度；一场对局内只解析一次，因此替补诊断也只记一次。

姿态与运动在两档[动画强度](#动画强度)下都保留，低强度只缩减幅度。演出不是攻防信息的唯一来源——连锁数与攻击量画在棋盘内，两者互不替代。运动词汇、分档阈值与时序不变量见[表现与 UI 设计 §3.1](../../presentation.md)。
