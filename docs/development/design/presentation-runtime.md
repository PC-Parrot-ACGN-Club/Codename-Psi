...
- 输入：该玩家的 Fever 状态、待接收垃圾、溢出风险、[文字反馈](#文字反馈)当前的行、小局与比赛结果，以及该角色的[表现数据](character-presentation.md)。
- 处理：按固定优先级取一个姿态——胜负 → Fever → 刚离场的攻击或抵消 → 溢出风险 → 有待接收垃圾 → 待机；和局不指定胜负方，因此落到下一项。姿态给出位移与缩放，位移方向朝向中央轨道。中央轨道显示最近一次攻防的文字，并按 `momentum` 的优势方偏移一格。
- 输出：两侧圆框的配色、徽章、位移与缩放，以及轨道上的文字与优势标记。
- 错误语义：角色表现目录不可用时用[替补](character-presentation.md#查询)绘制，只失去角色间的区分度；一场对局内只解析一次，因此替补诊断也只记一次。

姿态与位移在两档[动画强度](#动画强度)下都保留：轨道上的文字与数字是这段演出的唯一信息来源之外的那一份，两者互不替代。
