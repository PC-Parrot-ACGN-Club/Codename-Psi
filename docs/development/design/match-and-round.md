...
第 4 步释放的垃圾要按[垃圾落下](offense-and-nuisance.md#垃圾落下)占用重力时长。落下期间该玩家不持有活动组也不消费玩法动作，第 5、6 两步推迟到该批垃圾稳定的那个 tick 执行，因此出生检查读到的是垃圾停稳后的盘面。两步每个 `Playing` tick 各执行一次：等待供给的玩家可能来自本 tick 的安全点，也可能来自本 tick 落定的垃圾。
